...
async def analyze_listing(listing_id: int, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
...
async def analyze_all_listings(background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    """Запуск AI-анализа всех объявлений"""
    result = await db.execute(select(CarListing).where(CarListing.is_deleted == False))
    ...
    return {"status": "queued", "total": len(listings), "tasks": task_ids}
...
async def get_all_reports(
    ...
    # Базовый запрос
    query = select(CarListing).where(CarListing.is_deleted == False, CarListing.ai_verdict.isnot(None))
    ...
    return ReportsResponse(count=len(reports), total=total, page=page, per_page=limit, reports=reports)
