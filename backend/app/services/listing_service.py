...
class ListingService:
        ...
        for listing_data in listings:
            ...
            result = await db.execute(
                select(CarListing).where(
                    CarListing.external_id == listing_data["external_id"], CarListing.source == source.value
                )
            )
    ...
    async def get_listings(self, db: AsyncSession, skip: int = 0, limit: int = 20) -> List[CarListing]:
        ...
        result = await db.execute(select(CarListing).where(CarListing.is_deleted == False).offset(skip).limit(limit))
